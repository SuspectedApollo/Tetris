...
public abstract class Tetronimo {
	...
	protected Tetronimo(int[][] grid, Point[][] coords, int color){
		orient = 0;
		home = new Point(3,19);
	...
	protected Point[] getAbs(){
			...
			for(int i = 0; i < 4; i++){
				temp[i] = new Point((COORDS[orient][i].x + home.x), COORDS[orient][i].y + home.y);
			}
			return temp;
		}
	...
	protected void draw(){
		...
		for(int i = 0; i < 4; i++){
			grid[squares[i].x][squares[i].y] = COLOR;
		}
	}
